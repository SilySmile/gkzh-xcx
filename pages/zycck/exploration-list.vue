<template>
  <view class="page">
    ...
    <view class="section">
      ...
      <view v-if="showOther && !readOnly && items.length < 6" class="other-panel">
        ...
        <view v-for="item in filteredCareers" :key="item.careerId" class="other-row">
          <view class="item-content"><text class="item-name">{{ item.careerName }}</text><text class="item-intro">{{ item.oneLineIntro || '暂无一句话介绍' }}</text></view>
          <button v-if="!isAdded(item)" class="plus-button" type="primary" hover-class="button-hover" size="mini" @click="add(item)">＋</button>
...
export default {
  data: () => ({ recordId: '', instanceId: '', gameId: '', items: [], allCareers: [], viewedIds: [], readOnly: false, showOther: false, keyword: '' }),
  computed: {
    filteredCareers() { const key = this.keyword.trim(); return this.allCareers.filter(x => !key || String(x.careerName).includes(key)) },
  ...
  methods: {
    async load() { try { const [r, c] = await Promise.all([getExploration(this.recordId), getCatalog({ instanceId: this.instanceId, gameId: this.gameId })]); const d = r.data || {}; this.items = d.items || d.explorationItems || []; this.readOnly = this.readOnly || d.readOnly === true; const raw = d.viewedCareerIds || []; this.viewedIds = Array.isArray(raw) ? raw : JSON.parse(raw || '[]'); const rows = (c.data || {}).careers || []; this.allCareers = rows.filter(x => String(x.hasQuestion) !== '1').map(x => ({ ...x, careerId: x.careerId || x.careerQuestionId, careerName: x.careerName || x.name || x.title })) } catch (e) { uni.showToast({ title: userMessage(e, '探索清单加载失败，请重试'), icon: 'none' }) } },
    isAdded(i) { return this.items.some(x => String(x.careerId) === String(i.careerId)) },
    async add(i) { if (this.items.length >= 6) return uni.showToast({ title: '探索清单最多添加6个职业', icon: 'none' }); try { await addExplorationItem(this.recordId, i.careerId); await this.load() } catch (e) { uni.showToast({ title: userMessage(e, '加入清单失败，请重试'), icon: 'none' }) } },
