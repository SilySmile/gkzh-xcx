	...
	<scroll-view scroll-y class="page">
		...
		<view class="content">
			...
			<view class="awareness-card">
				...
				<button v-for="choice in awarenessChoices" :key="choice.value" class="awareness-choice"
					hover-class="awareness-hover" :class="{ 'awareness-pressed': pressedAwareness === choice.value }"
...
<script>
	...
	export default {
		...
		computed: {
			feedback() {
				return this.data.feedback || this.data || {}
			},
			guessedCareer() {
				return this.feedback.guessedCareer || this.data.guessedCareer || '未作答'
			},
			correctCareer() {
				return this.feedback.correctCareerName || this.data.correctCareer || '—'
			},
			answerCorrect() {
				return this.feedback.correct === true || this.feedback.correct === 'true'
			},
			questionNo() {
				return this.feedback.questionNo || this.data.questionNo || 1
			},
			careerImageUrl() {
				return this.feedback.careerImageUrl || ''
			},
			dayItems() {
		...
		methods: {
			...
			async loadFeedback() {
				try {
					...
					uni.showToast({
						title: userMessage(e, '反馈内容加载失败，请重试'),
						icon: 'none'
					})
				}
			},
			async submit(level) {
				...
				try {
					const r = await submitAwareness(this.recordId, {
						awareness: level
					})
					const d = r.data || {}
					...
					if (d.nextQuestion) {
						uni.redirectTo({
							url: `/pages/zycck/question?recordId=${this.recordId}&instanceId=${this.instanceId}&gameId=${this.gameId}&skipRules=1`
						})
					} else {
						uni.redirectTo({
