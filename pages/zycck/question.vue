<template>
	<view class="page">
		...
		<image v-if="question.questionImageUrl" class="picture" :src="imageUrl(question.questionImageUrl)"
			mode="aspectFill" />
		<text class="prompt">猜猜以上内容是下面哪个职业的工作场景？</text>
		<view v-for="(label, key) in options" :key="key" class="option" :class="optionClass(key)"
			:style="optionStyle(key)" @click="answer(key)">
			<text>{{ key }}. {{ label }}</text>
		...
		<view v-if="rulesVisible" class="mask">
			<view class="modal">
				<image
					class="rule-image"
					src="https://zhiye.sxgkzh.cn/imgs/zycck/gz1.png"
					mode="widthFix"
				/>
...
<script>
	...
	export default {
		...
		methods: {
			...
			optionStyle(key) {
				if (this.showResult && key === this.correctKey)
				return 'background-color:#bdecc9;border-color:#34a853;color:#146b32;'
				if (this.showResult && key === this.selectedKey && key !== this.correctKey)
				return 'background-color:#ffd0d0;border-color:#e34d59;color:#a61d2a;'
				if (!this.showResult && key === this.pressedKey)
				return 'background-color:#dceaff;border-color:#75aaf7;color:#174f98;'
				return ''
			},
			async load() {
				...
				try {
					...
					if (d.stage === 'feedback') {
						...
						return uni.redirectTo({
							url: `/pages/zycck/feedback?recordId=${this.recordId}&instanceId=${this.instanceId}&gameId=${this.gameId}`
						})
					}
					if (d.stage === 'summary') return uni.redirectTo({
						url: `/pages/zycck/summary?recordId=${this.recordId}&instanceId=${this.instanceId}&gameId=${this.gameId}`
					})
					...
					if (d.finished || d.status === 'finished') return uni.redirectTo({
						url: `/pages/zycck/exploration-list?recordId=${this.recordId}&instanceId=${this.instanceId}&gameId=${this.gameId}&readOnly=1`
					})
					if (!this.rulesVisible) {
						await this.ensureQuestionTimer();
						this.startTimer()
					}
				} catch (e) {
					uni.showToast({
						title: userMessage(e, '游戏进度加载失败，请重试'),
						icon: 'none'
					})
				}
			},
			...
			async ensureQuestionTimer() {
				...
				try {
					await startQuestionTimer(this.recordId)
				} catch (e) {
			...
			async timeout() {
				...
				try {
					const r = await submitAnswer(this.recordId, {
						questionId: this.question.questionId,
						questionNo: this.currentNo,
						timeoutFlag: true
					})
					this.advance(r.data || {})
				} catch (e) {
					uni.showToast({
						title: userMessage(e, '答题超时，正在进入下一题'),
						icon: 'none'
					})
					this.load()
				} finally {
			...
			advance(d) {
				const next = d.nextQuestion && Object.keys(d.nextQuestion).length ? d.nextQuestion : (d.question && Object
					.keys(d.question).length ? d.question : null)
				...
				if (next) {
					this.question = next;
					this.currentNo = d.currentQuestionNo || this.currentNo + 1
					this.remaining = Number(d.remainingSeconds == null ? 60 : d.remainingSeconds);
					this.timerStarted = false
					this.ensureQuestionTimer();
					this.startTimer()
...
<style scoped>
	...
	.timer {
		...
		background: #e8f1ff;
		color: #1b76fe;
		box-shadow: 0 6rpx 18rpx rgba(27, 118, 254, .12)
	}
	...
	.option {
		...
		border-radius: 14rpx;
		color: #243b53;
		box-shadow: 0 4rpx 14rpx rgba(31, 41, 55, .04);
		display: flex;
		justify-content: space-between;
		align-items: center;
